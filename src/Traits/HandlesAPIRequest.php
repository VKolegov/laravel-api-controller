...
trait HandlesAPIRequest
{
    ...
    /**
     * @param string|Builder|Relation|\Illuminate\Database\Eloquent\Builder $entityQualifier Либо имя класса, либо Query Builder либо отношение (Eloquent Relation)
     * @param Request $r
     * @param array $allowedFieldsToSearchBy Поля, по которым можно выполнять поиск. Пустой массив - без ограничений
     * @param array $additionalConditions Дополнительные условия для Model::where()
     * @param callable|null $mappingCallback
     * @param string|null $fieldsCase Как представлены поля в БД (snake/camel)
     * @return JsonResponse
     */
    protected function getSearchableEntities(
        $entityQualifier,
        Request $r,
        array $allowedFieldsToSearchBy = [],
        array $additionalConditions = [],
        callable $mappingCallback = null,
        string $fieldsCase = null
    ): JsonResponse
    {
        $filterOptions = $this->getSearchableEntitiesFilterOptions($r, $allowedFieldsToSearchBy, $additionalConditions);
    ...
    /**
     * Формирует Query Builder с учетом поискового запроса
     *
     * @param string|Builder|Relation $entityQualifier Либо имя класса, либо Query Builder либо отношение (Eloquent Relation)
     * @param Request $r
     * @param array $allowedFieldsToSearchBy
     * @param array $additionalConditions
     * @param string|null $fieldsCase
     * @return Relation|Builder|EloquentBuilder
     */
    protected function getSearchableEntitiesQuery(
        $entityQualifier,
        Request $r,
        array $allowedFieldsToSearchBy = [],
        array $additionalConditions = [],
    ...
    /**
     * Формирует параметр для Builder::where() с учётом поискового запроса
     * @param Request $r
     * @param array $allowedFieldsToSearchBy
     * @param array $additionalConditions
     * @return array
     */
    protected function getSearchableEntitiesFilterOptions(
        Request $r,
        array   $allowedFieldsToSearchBy = [],
        array   $additionalConditions = []
    ): array
    {
        if (count($allowedFieldsToSearchBy)) {
            $validationRules = [
                'searchBy' => [Rule::in($allowedFieldsToSearchBy)]
            ];
            $r->validate($validationRules);
        }


        $searchBy = $r->get('searchBy');
        $query = $r->get('q');

        $filterOptions = [];

        // Если есть какие-то дополнительные правила фильтрации - добавляем
        if ($additionalConditions) $filterOptions += $additionalConditions;

        // Если нет поискового запроса - не применяем фильтрацию
        if (!$query) {
